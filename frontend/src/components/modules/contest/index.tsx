...
import { ICourse } from '../manage-contest'
...
const items: MenuProps['items'] = [
  getItem('Tất cả cuộc thi', 'course', <AppstoreOutlined />),
  getItem('Cuộc thi tham gia', 'joinedCourse', <AppstoreOutlined />),
...
const ContestContent: NextPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const response1 = await axiosService.get('/course', { params: { is_contest: true } })
      const data1: ICourse[] = response1.data
      const response2 = await axiosService.get('/course/joined', { params: { is_contest: true } })
