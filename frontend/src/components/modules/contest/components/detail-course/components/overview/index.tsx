...
const OverviewCourse: NextPage<IProps> = props => {
  ...
  const handleClickButton = () => {
    Modal.confirm({
      title: 'Bạn có chắc muốn đăng ký cuộc thi này?',
      onOk: async () => {
        try {
          const res = await axiosService.post(`course/${props.course._id}/join`, {})
          props.reload()
          message.success('Đăng ký cuộc thi thành công')
        } catch (e) {
          message.error('Đăng ký cuộc thi thất bại')
        }
      },
      onCancel: () => {}
    })
  ...
  return (
    <div className={styles.content}>
      ...
      <Space className={styles.space_tab}>
        <Text className={styles.tab}>Giới thiệu cuộc thi</Text>
        <Text className={styles.tab}>Nội dung cuộc thi</Text>
        <Text className={styles.tab}>Giáo viên</Text>
      </Space>
      <Divider style={{ marginTop: 0 }} />
      {/*<div className={styles.description}>*/}
      {/*  <Title className={styles.title}>Giới thiệu về cuộc thi</Title>*/}
      {/*  <Text className={styles.text}>{props.course?.introduction}</Text>*/}
      {/*  <Title className={styles.title}>Nội dung cuộc thi</Title>*/}
